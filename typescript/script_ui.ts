...
const btnSpan = btnBookmark?.querySelector("span");
...
if (
	...
	Array.from(pledgeCards).forEach(card => {
		const checkbox: HTMLInputElement | null = card.querySelector(".check");
		...
		card.addEventListener("click", () => {
			if (card.parentElement) {
				...
				else card.parentElement.classList.remove("selected");
			}
		});
	});
